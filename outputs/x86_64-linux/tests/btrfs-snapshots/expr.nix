...
in
{
    ...
    (
      _name: system:
        let
          ...
          expectedSources =
            lib.optional (isSnapshotPair config "/persistent" "/snapshots") "/persistent"
            ++ lib.optional (isSnapshotPair config "/data/apps" "/data/apps-snapshots") "/data/apps";
  ...
  ovhAppsSnapshotPair = isSnapshotPair ovhConfig "/data/apps" "/data/apps-snapshots";
  serverAppsSnapshotPair = isSnapshotPair serverConfig "/data/apps" "/data/apps-snapshots";
  ovhReservedDataMounts =
    let
      appsDevice = ovhConfig.fileSystems."/data/apps".device;
    in
    lib.all
      (
        mountPoint:
        builtins.hasAttr mountPoint ovhConfig.fileSystems
        && ovhConfig.fileSystems.${mountPoint}.fsType == "btrfs"
        && ovhConfig.fileSystems.${mountPoint}.device == appsDevice
      )
      [
        "/data/fileshare"
        "/data/backups"
