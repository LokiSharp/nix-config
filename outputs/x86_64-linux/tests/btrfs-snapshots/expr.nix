...
lib.mapAttrs
  (
    ...
    let
      ...
      expectedSources = lib.optional (isSnapshotPair "/persistent" "/snapshots") "/persistent"
    ...
    in
    {
      instanceMatchesFilesystems =
        builtins.hasAttr "local" config.services.btrbk.instances == enabled;
      ...
      destinationsMatchFilesystems = lib.all
        (
          ...
          in
          if source == "/persistent" then
            settings.snapshot_dir == "/snapshots"
            && settings.snapshot_name == "persistent"
          else
            settings.snapshot_dir == "/data/apps-snapshots"
            && settings.snapshot_name == "apps"
        )
        expectedSources;
      retentionPolicyConfigured =
        ...
        || (
          ...
          && lib.all
            (source: instance.settings.subvolume.${source}.snapshot_create == "always")
            expectedSources
        );
      mountDependenciesComplete =
        !enabled
        || lib.sort builtins.lessThan
          config.systemd.services.btrbk-local.unitConfig.RequiresMountsFor
